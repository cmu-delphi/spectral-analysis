...
def detect_schema(df: pd.DataFrame):
    ...
    if time_col and len(numeric_cols) >= 1:
        ...
        raise ValueError(
            "Could not detect a usable schema. Provide either:\n"
            " - wide schema with columns: geo_value, time_value, and ≥1 numeric signals\n"
            " - simple series with a numeric column (and optional time/date column)"
        )
...
def preprocess_signal(y, t_raw, pad_length, pad_side="both"):
    """Zero-pad for FFT math and create corresponding full timeline."""
    if pad_side == "left":
...
def pad_for_plot(y, pad_length, pad_side="both"):
    """
    Zero-pad the series for visualization so padding is visibly 0 on the plot.
    """
    left = [0.0] * pad_length
...
def apply_frequency_filter(freqs, fft_vals, low_cutoff, high_cutoff, method="hard"):
    ...
    if method == "gaussian":
        ...
        fft_filtered[
            (np.abs(freqs) < low_cutoff) | (np.abs(freqs) > high_cutoff)
        ] = 0
    return fft_filtered
...
def create_fft_plot(full_time, y_plot_original, y_plot_recon,
    ...
    fig.add_trace(
        go.Scatter(x=full_time, y=y_plot_recon, name=recon_label, line=dict(color="blue")),
        row=1, col=1
    )
    ...
    fig.add_trace(go.Scatter(
        x=sorted_periods, y=sorted_magnitude, mode='markers',
...
def generate_fft_figure(df, state, signal, pad_length=10,
    ...
    y_plot_original = pad_for_plot(y_original_for_plot, pad_length, pad_side)
...
def init_or_upload(contents, filename):
    if contents is not None and filename:
        try:
            ...
            return (
                default_schema["df"].to_dict("records"),
...
def populate_dropdowns(schema):
    ...
    state_options = [{"label": s.upper(), "value": s} for s in states]
    signal_options = [{"label": s, "value": s} for s in signals]
    return state_options, (states[0] if states else None), signal_options, (signals[0] if signals else None)
